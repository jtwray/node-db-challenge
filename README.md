...
SQL or Structured Query Language is used in Relational Databases to organize large amounts of data. THere are Many different Relation Databases . THere is one SQL.

- [ ] Why do tables need a `primary key`?

THe primary key of each row on a table is a unique identifying seqeuential number used to separate all the rows from one another. The primary key is also referneced by others tables by the term foreign key allowing table inter relations to be made.
...
a bridge table is one way to foster a connection between two plural resources. many to many.
